...
NS_OBJECT_ENSURE_REGISTERED (TraceBasedPropagationLossModel);

TypeId 
TraceBasedPropagationLossModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TraceBasedPropagationLossModel")
    .SetParent<PropagationLossModel> ()
    .SetGroupName ("Propagation")
    .AddConstructor<TraceBasedPropagationLossModel> ()
    .AddAttribute ("NrTraceBasedNodes", "The number of trace based nodes.",
                   UintegerValue (2),
                   MakeUintegerAccessor (&TraceBasedPropagationLossModel::m_nrTraceBasedNodes),
                   MakeUintegerChecker<uint32_t> ())
  ;
  return tid;
}
TraceBasedPropagationLossModel::TraceBasedPropagationLossModel ()
  : PropagationLossModel ()
{
    //initialize Rss 2D arrays

    for(int i=0; i<100; i++)
    {
       for(int j=0; j<100; j++)
       {
            m_2dRssArray[i][j] = -999.0; //RSS value too low to be true (dBm)
       }
    }      
}
...
void 
TraceBasedPropagationLossModel::InitMobilityModelForEachNode (void)
{
    Config::MatchContainer matchCont = Config::LookupMatches("/NodeList/*/$ns3::MobilityModel");
    NS_LOG_UNCOND("Number of nodes found");
    NS_LOG_UNCOND(matchCont.GetN());
    m_nrTraceBasedNodes = matchCont.GetN();

    for(int i=0; i<m_nrTraceBasedNodes; i++)
    {
        m_mobilityModelsArray[i] = DynamicCast<MobilityModel> (matchCont.Get(i));
    }  
} 

void 
TraceBasedPropagationLossModel::SetRssOfNodeXFromNodeY (double rss, uint32_t receiverNodeX, uint32_t senderNodeY)
{
  m_2dRssArray[receiverNodeX][senderNodeY] = rss;
}

double
TraceBasedPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                  Ptr<MobilityModel> a,
                                  Ptr<MobilityModel> b) const
{
    int senderIndex=-1;
    ...
    return m_2dRssArray[receiverIndex][senderIndex];
}

int64_t
TraceBasedPropagationLossModel::DoAssignStreams (int64_t stream)
{
  return 0;
}
