 ...
 * \ingroup propagation
 *
 * \brief Return a predefined received power level independent of the transmit 
 *  power, based on real node traces
 *
 * The received power is configurable by the user, based on real node traces, 
 * and independent of the transmit power.  In this poof-of-concept version, the 
 * user must set the current received power level for a fixed node and a mobile
 * node. The user must set the these variables throughout the simulation, setting 
 * the RssFixedNode and RssMobileNode attributes or public SetRssFixedNode() and 
 * SetRssMobileNode() methods.
 * The user must also set the Vector representing the FixedNode position throught
 * the FixedNodePosition attribute.  
 * Note that if this loss model is chained to other loss
 * models via SetNext() method, it can only be the first loss model in such
 * a chain, or else it will disregard the losses computed by loss models
 * that precede it in the chain. 
 */ 
class TraceBasedPropagationLossModel : public PropagationLossModel
{
public:
  /**
   * \brief Get the type ID.
  ...
  virtual ~TraceBasedPropagationLossModel ();
  
  
  /**
  * Initialize the array of mobility models for each node id.
  */
  void InitMobilityModelForEachNode (void); 
  /**
  * \param nrNodes (uint32_t) the number of Trace Based Nodes 
  *
  * Set the number of Trace Bases Nodes.
  */
//  void SetNrTraceBasedNodes (uint32_t nrNodes); //Não deve ser necessária
  /**
  * \param rss (double) the receivedNodeX signal strength
  * \param receiverNodeX (uint32_t) the number of the receiver node
  * \param senderNodeY (uint32_t) the number of the sender node 
  *
  * Set the received signal strength (RSS) in dBm of receiverNodeX for a signal sent from senderNodeY.
  */
  void SetRssOfNodeXFromNodeY (double rss, uint32_t receiverNodeX, uint32_t senderNodeY);
...
private:
   ...
   * Defined and unimplemented to avoid misuse
   */
  TraceBasedPropagationLossModel (const TraceBasedPropagationLossModel &);
  /**
   * \brief Copy constructor
   *
   * Defined and unimplemented to avoid misuse
   * \returns
   */
  TraceBasedPropagationLossModel & operator = (const TraceBasedPropagationLossModel &);
  ...
  //Helder 2018
  int m_nrTraceBasedNodes; //!< the number of trace based nodes
  double m_2dRssArray[100][100]; //!< 2D Array containing the Rss between all nodes
  Ptr<MobilityModel> m_mobilityModelsArray[100]; //!< Array containing the Ptr for the mobility model of each node 
};
