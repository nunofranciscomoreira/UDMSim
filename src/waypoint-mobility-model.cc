...
TypeId WaypointMobilityModel::GetTypeId(void) {
	static TypeId tid = TypeId("ns3::WaypointMobilityModel").SetParent<
			MobilityModel>().SetGroupName("Mobility").AddConstructor<
			WaypointMobilityModel>().AddAttribute("NextWaypoint",
			"The next waypoint used to determine position.", TypeId::ATTR_GET,
			WaypointValue(),
			MakeWaypointAccessor(&WaypointMobilityModel::GetNextWaypoint),
			MakeWaypointChecker()).AddAttribute("WaypointsLeft",
			"The number of waypoints remaining.", TypeId::ATTR_GET,
			UintegerValue(0),
			MakeUintegerAccessor(&WaypointMobilityModel::WaypointsLeft),
			MakeUintegerChecker<uint32_t>()).AddAttribute("LazyNotify",
			"Only call NotifyCourseChange when position is calculated.",
			BooleanValue(false),
			MakeBooleanAccessor(&WaypointMobilityModel::m_lazyNotify),
			MakeBooleanChecker()).AddAttribute("InitialPositionIsWaypoint",
...
void WaypointMobilityModel::DoDispose(void) {
...
Waypoint WaypointMobilityModel::GetNextWaypoint(void) const {
	Update();
	return m_next;
}
uint32_t WaypointMobilityModel::WaypointsLeft(void) const {
	Update();
	return m_waypoints.size();
}
void WaypointMobilityModel::Update(void) const {
...
Vector WaypointMobilityModel::DoGetPosition(void) const {
	Update();
...
void WaypointMobilityModel::EndMobility(void) {
	m_waypoints.clear();
	m_current.time = Time(std::numeric_limits<uint64_t>::infinity());
	m_next.time = m_current.time;
	m_first = true;
}
void WaypointMobilityModel::ClearMobility(void) {
	Time now = Simulator::Now();
	m_waypoints.clear();

	m_current.time = Time(now - 2);
	m_next.time = m_current.time;
	m_first = true;
	Update();
}
Vector WaypointMobilityModel::DoGetVelocity(void) const {
	return m_velocity;
}
