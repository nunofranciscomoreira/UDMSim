...
public:
  ...
  void SetMaxBytes (uint64_t maxBytes);
   /**
   *\param maxRetries the upper bound of send retries
   */
 // void SetMaxRetries (uint64_t maxRetries);
  /**
...
private:
  ...
  TracedCallback<uint32_t> m_error_close;
...
private:
  ...
  /**
   * \brief Connection Closed with no error (called by Socket through a callback)
   * \param socket the disconnected socket
   */
  void NormalClose (Ptr<Socket> socket);
    /**
   * \brief Connection Closed with error (called by Socket through a callback)
   * \param socket the disconnected socket
   */
  void AbnormalClose (Ptr<Socket> socket);
